    ...
    private inner class ReadTask(private val isoDep: IsoDep, private val bacKey: BACKeySpec) : AsyncTask<Void?, Void?, Exception?>() {
        ...
        override fun doInBackground(vararg params: Void?): Exception? {
            try {
                ...
                try {
                    ...
                    Log.w(TAG, "Skipping DG2 for custom milestone card", e)
                }
                try {
                    val sodIn = service.getInputStream(PassportService.EF_SOD)
                    sodFile = SODFile(sodIn)
                } catch (e: Exception) {
                    Log.w(TAG, "Skipping SOD/passive authentication for custom milestone card", e)
                }
        ...
        private fun doChipAuth(service: PassportService) {
            try {
                ...
                val dg14 = DG14File(dg14InByte)
                dg14File = dg14
                val dg14FileSecurityInfo = dg14.securityInfos
                val chipAuthenticationInfo = dg14FileSecurityInfo
                    .filterIsInstance<ChipAuthenticationInfo>()
                    .firstOrNull()
                for (securityInfo: SecurityInfo in dg14FileSecurityInfo) {
                    if (securityInfo is ChipAuthenticationPublicKeyInfo && chipAuthenticationInfo != null) {
                        service.doEACCA(
                            securityInfo.keyId,
        ...
        private fun doPassiveAuth() {
            try {
                val sodFile = sodFile ?: return
                val dg2File = dg2File ?: return
                ...
                val dg1HashMatches = dg1Hash.contentEquals(dataHashes[1])
